...
# ============================================================
# 0. ADB 工具 (提速用: 跳过 uiautomator2 中间层, 1.3s vs 2-3s/次)
#    只用于主循环"取列表信息", 详情页保持 d().wait() (避免 dump 缓存坑)
# ============================================================
def adb_dump_xml(adb_path, device):
    """ADB 直接 dump UI XML, ~1.3s/次"""
    try:
        subprocess.run(
...
def _find_nodes(xml, resource_id):
    """找所有 resource-id 匹配的 node, 返回 list of attrs dict"""
    nodes = []
...
def xml_get_first_row_date(xml):
    """取列表第一行日期 (MM-DD HH:MM:SS 格式)
    优先 tv_time 节点 (老版本能拿到),
    兜底搜整个 XML 第一个匹配 MM-DD HH:MM:SS 的 text 字段
    """
...
def xml_get_first_row_amount(xml):
    """取列表第一行 tv_amount 文本 (Y>500 过滤, 避开资产卡片)"""
...
def xml_page_state(xml):
    """从 XML 判断页面状态: 'list' / 'detail' / 'unknown'"""
...
def main():
    ...
    no_date_count = 0  # 连续拿不到日期的次数, 防止 list_md 永远 None 时不停止
    try:
        for i in range(args.max_items):
            # === 1. 检查列表页状态 (uiautomator2 d().exists, 不用 dump) ===
            ...
            # === 2. 取列表第一行的日期 (uiautomator2, 不走 dump) ===
            list_date = get_first_row_date(d)
            ...
            if list_md:
                ...
                no_date_count += 1
                if no_date_count >= 3:
                    print(f'  [{i+1}] 连续 {no_date_count} 次拿不到日期, 视为到底, STOP')
                    stopped = True
                    break

            # === 4. 取首行金额 (uiautomator2, 不走 dump) ===
            amount = get_first_row_amount(d)
            if not amount:
            ...
            # === 10. 返回列表 (跳过固定 0.8s, 短 sleep + 一次校验) ===
            tap_back(d)
            time.sleep(0.3)
            if not is_list_page(d):
                # 兜底: 再 back 一次
